...
base_layer.fill(colorWHITE)  # Fill background with white
...
while running:
    ...
    if draw:
        if mode == 'circle':
            ...
            drawSquare(screen, start_pos, end_pos, radius, color)
        elif mode == 'right_triangle':
            drawRightTriangle(screen, start_pos, end_pos, color)
        elif mode == 'equilateral_triangle':
            drawEquilateralTriangle(screen, start_pos, end_pos, radius, color)
        elif mode == 'rhombus':
            drawRhombus(screen, start_pos, end_pos, radius, color)
        elif mode == 'erase':
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION and draw:
            end_pos = event.pos  # Update in real time

        if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and draw:
            end_pos = event.pos
            # Draw final version onto base layer
            if mode == 'circle':
